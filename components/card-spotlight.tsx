...
export function CardSpotlight() {
  return (
    <div className="relative flex h-full min-h-[390px] min-w-0 items-center justify-center overflow-hidden sm:min-h-[440px] lg:min-h-[520px]">
      <div
        className="absolute inset-x-[8%] bottom-[9%] h-[24%] rounded-[50%] bg-blue-950/45 blur-2xl"
        aria-hidden="true"
      />

      <div className="absolute inset-x-0 top-[4%] z-10 grid grid-cols-2 items-end gap-1 px-3 sm:top-[2%] sm:gap-2 sm:px-10 lg:px-12">
      ...
      <div className="absolute left-1/2 top-[55%] z-30 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center gap-2 sm:top-[52%] sm:gap-3">
        {looseCards.map((card) => (
          <figure className="shrink-0" key={card.src}>
            <Image
              alt={card.alt}
              className={`h-auto w-[72px] object-contain drop-shadow-[0_18px_16px_rgba(3,7,18,0.65)] transition duration-300 hover:z-40 hover:-translate-y-2 hover:rotate-0 sm:w-[94px] lg:w-[108px] ${card.className}`}
              height={367}
              priority
      ...
      <Image
        ...
        className="absolute bottom-[2%] left-1/2 z-20 h-auto max-h-[128px] w-auto max-w-[54%] -translate-x-1/2 object-contain drop-shadow-[0_22px_18px_rgba(3,7,18,0.55)] transition duration-300 hover:-translate-y-2 sm:bottom-[1%] sm:max-h-[175px] sm:max-w-[48%] lg:max-h-[195px]"
        height={533}
        priority
